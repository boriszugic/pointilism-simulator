# **Pointilism Simulator**
### Description
Program that produces a pointilistic image of the given image. More on pointilism https://en.wikipedia.org/wiki/Pointillism.
### How to execute
Enter the following in a console directed to location of your image: python pointilism.py {filename} {OPTIONAL: filename2}
	filename - name of image file to be converted
	filename2 - output image will be saved under this name in current directory
### Authors and Acknowledgment
- Boris Zugic, boriszugic123@gmail.com